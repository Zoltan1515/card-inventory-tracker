...
assert(page.includes('{expenseModalOpen && (') && page.includes('aria-label={expenseForSoldCard ? `Add expense for ${expenseForSoldCard.name}` : editingExpenseId ? "Edit expense" : "Add expense"}'), 'Add/edit expense should render in a modal dialog.');
