...
assert(page.includes('Back photo for eBay'), 'Add inventory form should show a back-photo upload option for eBay.');
assert(page.includes('Add/replace back photo for eBay'), 'Edit card form should show a back-photo upload option.');
...
assert(primeLotRoute.includes('image_url_back: card.backPhotoUrl?.trim() || null'), 'PrimeLot posting should pass the back photo into image_url_back.');
