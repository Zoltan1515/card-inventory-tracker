...
assert(mapper.includes('purchasePriceAliases') && mapper.includes('purchasePriceFromText') && mapper.includes('originalPurchasePrice') && mapper.includes('original_price') && mapper.includes('pricePaid') && mapper.includes('costBasis') && mapper.includes('acquisitionCost'), "Mapper accepts PrimeLot purchase-price aliases and parses WCT-exported purchase price text.");
...
assert(mapper.includes('asking_price: firstNumber(listing.price, listing.askingPrice, listing.asking_price, listing.listPrice, listing.list_price)') && mapper.includes('purchase_price: purchasePriceForListing(listing)'), "Mapper imports PrimeLot price as WCT asking/listing price and keeps purchasePrice only as cost basis.");
assert(route.includes('refreshDuplicateMissingListingPrice') && route.includes('filled missing PrimeLot listing price') && route.includes('mappedAskingPrice'), "Import route refreshes duplicate rows that previously missed the PrimeLot listing/asking price and logs mapped asking price diagnostics.");
...
assert(route.includes('findUniqueWctOriginByExactNameAndAskPrice') && route.includes('same name and PrimeLot asking price') && route.includes('ambiguous'), "Import route safely recovers legacy draft imports from one exact WCT name/asking-price match, but skips ambiguous matches.");
...
assert(route.includes('uuidPattern') && route.includes('uuidPattern.test(lastPathPart)'), "PrimeLot DB id lookups must only use UUID-shaped candidates so a full listing URL cannot make Supabase reject the source-row lookup.");
